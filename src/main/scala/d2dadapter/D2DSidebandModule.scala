...
class D2DSidebandModule(val fdiParams: FdiParams, val sbParams: SidebandParams) extends Module{
    ...
    when(sideband_switch.io.inner.node_to_layer_below.valid && sideband_switch.io.inner.node_to_layer_below.ready){
        ...
        }.elsewhen(SBMsgCompare(sideband_switch.io.inner.node_to_layer_below.bits, SBM.ADV_CAP)){
    ...
    when(io.sideband_snt =/= SideBandMessage.NOP){
        when(io.sideband_snt === SideBandMessage.REQ_ACTIVE){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_REQ_ACTIVE, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.REQ_L1){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_REQ_L1, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.REQ_L2){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_REQ_L2, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.REQ_LINKRESET){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_REQ_LINKRESET, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.REQ_DISABLED){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_REQ_DISABLE, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.RSP_ACTIVE){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_RSP_ACTIVE, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.RSP_PMNAK){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_RSP_PMNAK, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.RSP_L1){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_RSP_L1, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.RSP_L2){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_RSP_L2, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.RSP_LINKRESET){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_RSP_LINKRESET, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.RSP_DISABLED){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.LINKMGMT_ADAPTER0_RSP_DISABLE, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.PARITY_FEATURE_REQ){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.PARTIYFEATURE_REQ, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.PARITY_FEATURE_ACK){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.PARTIYFEATURE_ACK, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.PARITY_FEATURE_NAK){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.PARTIYFEATURE_NAK, src = "D2D", dst = "D2D", remote = true)
        }.elsewhen(io.sideband_snt === SideBandMessage.ADV_CAP){
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.ADV_CAP, src = "D2D", dst = "D2D", remote = true, data = D2DSidebandConstant.ADV_CAP_MESSAGE_DATA)
        }.otherwise{
            sideband_switch.io.inner.layer_to_node_below.bits := SBMsgCreate(base = SBM.NOP_CRD, src = "D2D", dst = "D2D", remote = true)
        }
