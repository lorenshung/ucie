...
class SidebandLinkNode(val sbParams: SidebandParams, val fdiParams: FdiParams)
  ...
  val numDesTimeoutCycles = 512
  val tx_ser =
    Module(new SidebandLinkSerializer(sbParams.sbLinkWidth, sbParams.sbNodeMsgWidth))
  val rx_des =
    Module(
  ...
  private val rxtxMode = Mux(io.rxMode === RXTXMode.RAW, SBRxTxMode.RAW, SBRxTxMode.PACKET)
  tx_ser.io.ctrl.tx_mode := rxtxMode
  rx_des.io.ctrl.rx_mode := rxtxMode

  // Connect outer signals
  io.outer.tx.bits := tx_ser.io.out.bits
  io.outer.tx.clock := tx_ser.io.out.clock

  rx_des.io.in.bits := io.outer.rx.bits
  rx_des.io.in.fw_clock := io.outer.rx.clock
